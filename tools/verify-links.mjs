...
async function markdownFiles(root, relative = "") {
  ...
  for (const entry of await readdir(path.join(root, relative), { withFileTypes: true })) {
    ...
    // Historical decisions/plans retain references to the implementation at that time.
    if (["docs/decisions", "docs/superpowers/completed"].includes(child.split(path.sep).join("/"))) continue;
